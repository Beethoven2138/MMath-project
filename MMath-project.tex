...
\author{Saxon Supple\\Supervisor: Johannes Nordstr\"{o}m}
\date{April 2024}
...
\begin{abstract}
\noindent I would like to thank Johannes Nordstr\"{o}m for the considerable help and support he provided throughout this project, and for being an excellent lecturer in last semester's algebraic topology module.
\end{abstract}

\tableofcontents

\section{Introduction}
Algebraic topology seeks to distinguish topological spaces up to homeomorphism and homotopy equivalence by associating algebraic structures to topological spaces which are isomorphic when the spaces are homeomorphic or homotopy-equivalent. Most invariants are homotopy invariants, with homeomorphic being a stronger condition than homotopy-equivalent. The simplest example would be the number of conneted-components -- a homotopy invariant. The most commonly used homotopy invariants are the fundamental group, the homology groups and the cohomology groups. This project will first study singular homology, which is the simplest and most general homology theory to study, but has the disadvantage of being difficult to perform actual computations on, due to being defined in terms of a potentially uncountably infinite number of continuous maps. The singular homology groups are defined by forming abelian groups, known as chain groups, from continuous maps of a certain dimension into the topological space which have no boundary, and quotienting by the boundaries of the maps of a dimension one higher; this makes sense because the boundary of a boundary is $0$, as will be proven. We will then see a connection between the fundamental group and the first homotopy group through the Hurewicz theorem, which roughly says that the first homology group is the abelian version of the fundamental group.\\

\noindent We will then study the cohomology groups, which are dual to the homology groups. They have the advantage of admitting a ring structure through the cup product, and so have additional structure, giving a finer invariant. We will also see the universal coefficient theorem and Poincar\'e duality, which are results which connect the homology and cohomology groups. Poincar\'e duality however only holds for closed oriented manifolds, however, and is given by an isomorphism involving the cap product and a fundamental class; hence these concepts will also be discussed in detail. Orientation on manifolds is a generalized notion of consistent choice of unit normal for surfaces in $\mathbb{R}^3$ and can be defined in several ways. Here we shall study the homological definition, which involves a continuous choice of generator of the local homology group at each point on a manifold; local homology is a special case of relative homology: a homology given by "modding out" by a subspace of a topological space. It turns out that closed manifolds, that is manifolds without boundary, admit a simpler definition of orientability: an n--dimensional manifold is orientable if and only if it's n'th homology group is isomorphic to $\mathbb{Z}$. A generator of this group is termed a fundamental class, and represents an orientation of a manifold. Poincar\'e duality then relates the $k$'th homology group and $(n-k)$'th cohomology of an $n$--manifold through an explicit isomorphism given in terms of a fundamental class.\\

\noindent We will then use these objects to attempt to extract topological information about lens spaces, which are quotient spaces of the $3$--sphere by $\mathbb{Z}_p$, where the quotient space is given by the relation that two points are equivalent if they are in the same orbit of the group action. To state it explicitly, if $S^3$ is given by \[S^3:=\{(u,v)\in\mathbb{C}^2:|u|^2+|v|^2=1\}\] and $q$ is coprime to $p$, then the lens space $L(p;q)$ is the quotient space given by the action generated by \[(v,w)\mapsto(e^{2\pi i/p}v,e^{2\pi iq/p}w).\]

\noindent 
We will see the following result for the fundamental group, homology groups and cohomology groups of lens spaces:
\[\pi_1(L(p;q))\cong\mathbb{Z}_p,\text{ }H_k(L(p;q),\mathbb{Z})\cong\begin{cases}
       \mathbb{Z} &\quad\text{if }k=0 \\
       \mathbb{Z}_p &\quad\text{if }k=1 \\
       0 &\quad\text{if }k=2 \\
       \mathbb{Z}&\quad\text{if }k=3\\
       0&\quad\text{otherwise}\\
     \end{cases},\text{ }H^k(L(p;q),\mathbb{Z})\cong\begin{cases}
       \mathbb{Z} &\quad\text{if }k=0 \\
       0 &\quad\text{if }k=1 \\
       \mathbb{Z}_p &\quad\text{if }k=2 \\
       \mathbb{Z}&\quad\text{if }k=3\\
       0&\quad\text{otherwise}\end{cases}\]\\

\noindent These groups only depend on $p$, which turns out to be insufficient. We will introduce two new invariants: the torsion linking form and Reidemeister torsion. The torsion linking form is a homotopy-invariant and is defined on the torsion part of the second cohomology group, where the torsion subgroup consists of all the elements of finite order. However, the torsion linking form requires explicitly defined cochains (homomorphisms from chains to an abelian group) and as such requires a simpler set of chains than the uncountably infinitely many which singular homology gives. To this end we shall introduce Delta complexes, which are simple enough to allow for lens spaces to be constructed in terms of finitely many chains, yet still allow for cup products, and hence the torsion linking form, with which it is defined, to be computed. The homeomorphism classification then requires Reidemeister torsion, which is a homeomorphism invariant -- but is not a homotopy invariant -- of cell complexes, where a cell complex is a topological space built combinatorially by beginning with a set of points and iteratively attaching higher-dimensional balls along their boundaries. A Delta complex is a special case of a cell complex, with additional constraints which allow for cup products to be computed, albeit at the cost of added complexity in their construction. The Reidemeister torsion for cell complexes arose out of a more general form for exact sequences of modules with coefficients in a commutative ring; a module refers to a generalisation of a vector space, where the the field is replaced by a ring, and an exact sequence refers to a sequence of modules \[...\overset{f_n}{\to}C_n\overset{f_{n-1}}{\to} C_{n-1}\overset{f_{n-2}}{\to} C_{n-2}\overset{f_{n-3}}{\to}...\] where the image of $f_i$ is the kernel of $f_{i-1}$. The Reidemeister torsion is then formed as the products of the determinants of change of basis matrices, and so the computation of the Reidemeister torsion of a cell complex is the easiest when the modules are $1$--dimensional, since the change of basis matrices will be $1$ by $1$. Thankfully, it is possible to construct such a cell complex structure of a lens space if we forego the delta complex structure. The torsion linking form and Reidemeister torsion comprise the two necessary conditions needed for the homotopy and homeomorphism classification; and although the sufficiency of the condition obtained from the torsion linking form is beyond the scope of this project (it involves the higher homotopy groups and the Whitehead theorem\cite{Hatcher}, which says that a continuous map between cell complexes inducing isomorphisms on all homotopy groups is enough to make it a homotopy equivalence), it is relatively straightforward to construct the homeomorphisms which show the sufficiency of the condition given by the Reidemeister torsion. We will then obtain the following classification:\\

\noindent Let $L(p;q_1)$ and $L(p;q_2)$ be lens spaces.\\
...
\noindent From this we can see that $L(7;1)$ and $L(7;2)$ are homotopy-equivalent but not homeomorphic, which gives the simplest example of manifolds which are homotopy-equivalent but not homeomorphic.


\section{Homology}
\subsection{Background}
We begin by recalling some basic definitions and results from algebraic topology.\cite{alg}
\begin{definition}
...
\begin{definition}
Let $X$ be a topological space and $x_0\in X$. The \textbf{fundamental group} is \[\pi_1(X,x_0)=\{[\gamma]:\gamma\text{ is a loop based at }x_0\}.\] under the relation that $\gamma\sim\gamma'$ if $\gamma$ is based homotopic to $\gamma'$.
\end{definition}
...
\noindent Some concepts from algebra also needs to be introduced:
...
\begin{definition}
A \textbf{chain complex} is a sequence of abelian groups $...,A_0,A_1,A_2,A_3,...$ connected by homomorphisms such that the composition of any two consecutive homomorphisms is the zero map.
\end{definition}

\subsection{Singular homology}
\begin{definition}
A \textbf{k-simplex} is the convex hull\[[v_0,...,v_k]:=\big\{\sum\limits_{i=0}^k\lambda_iv_i:\lambda_i\in [0,1],\sum\limits_{i=0}^k\lambda_i=1\big\}\]
of $k+1$ linearly independent vectors $v_0,...,v_k\in\mathbb{R}^n$.\\
The \textbf{standard k-simplex} is\[\Delta^k=[e_0,...,e_k]=\big\{(t_0,...,t_k)\in\mathbb{R}^k:t_i\geq 0, \sum\limits_{i=0}^kt_i=1\big\}.\]
The vectors $v_0,...,v_k$ define a \textbf{framing} \[F_{v_0,...,v_k}\colon\Delta^k\rightarrow [v_0,...,v_k],(t_i)\mapsto \sum\limits_{i=0}^kt_iv_i.\]
\end{definition}
...
\begin{definition}
Let $X$ be a topological space. A \textbf{singular k-simplex} in X is a continuous map $\sigma\colon\Delta^k\to X$. The \textbf{k-th chain group} of $X$ is the abelian group\[C_k(X):=\{\sum\limits_{i=0}^kn_i\sigma_i:k\in\mathbb{N}_0,n_i\in\mathbb{Z},\sigma_i \text{ is a k-simplex}\}\] where the elements are called \textbf{k-chains}.
\end{definition}
\begin{remark}
$C_k(X)$ is an example of a \textbf{free abelian group}; that is, an abelian group whose elements are of the form $\sum\limits_in_is_i$ where each $s_i$ belongs to some set $S$.
\end{remark}


\begin{definition}
The \textbf{boundary map} $\partial=\partial_k\colon C_k(X)\to C_{k-1}(X)$ is given by\[\partial\sigma=\sum\limits_{i=0}^k(-1)^i\sigma\circ F_{e_0,...,\overset{\wedge}{e_i},...,e_k}\in C_{k-1}(X)\]
...
Let $\sigma\in C_k(X)$.
\begin{align*}
\partial\circ\partial (\sigma)&=\partial(\sum\limits_{i=0}^k(-1)^i\sigma\circ F_{[e_0,...,\overset{\wedge}{e_i},...,e_k]})\\
&=\sum\limits_{i=0}^k(-1)^i\partial\circ\sigma\circ F_{[e_0,...,\overset{\wedge}{e_i},...,e_k]} \\&\text{(by linearity)}\\
&=\sum\limits_{i=0}^k(\sum\limits_{j<i}(-1)^{i+j}\sigma\circ F_{[e_0,...,\overset{\wedge}{e_j},...,\overset{\wedge}{e_i},...,e_k]} + \sum\limits_{i<j}(-1)^{i+j-1}\sigma\circ F_{[e_0,...,\overset{\wedge}{e_i},...,\overset{\wedge}{e_j},...,e_k]}) \\&\text{(since }e_j \text{ gets shifted forward one spot when }e_i\text{ is removed first)}\\
&=\sum\limits_{i=0}^k(\sum\limits_{j<i}(-1)^{i+j}\sigma\circ F_{[e_0,...,\overset{\wedge}{e_j},...,\overset{\wedge}{e_i},...,e_k]} + \sum\limits_{j<i}(-1)^{i+j-1}\sigma\circ F_{[e_0,...,\overset{\wedge}{e_j},...,\overset{\wedge}{e_i},...,e_k]})
\\&\text{(interchanging }j\text{ with }i\text{ in the second term)}\\
&=\sum\limits_{i=0}^k(\sum\limits_{j<i}((-1)^{i+j}+(-1)^{1+j-1})\sigma\circ F_{[e_0,...,\overset{\wedge}{e_j},...,\overset{\wedge}{e_i},...,e_k]})=0
\end{align*}\qedhere
\end{proof}

\noindent This corresponds to the fact that the boundary of a ball is a sphere, which in turn has no boundary.\\
...
\begin{definition}
The k-th singular chain group with coefficients in an abelian group $G$ is \[C_k(X;G):=\{\sum\limits_{i=1}^kn_i\sigma_i:k\in\mathbb{N}_0,n_i\in G,\sigma_i \text{ is a k-simplex}\}.\] The boundary map $\partial\colon C_k(X;G)\to C_{k-1}(X;G)$ and k-th homology group with coefficients in $G$, $H_k(X;G)$, are then defined as above while replacing $\mathbb{Z}$ with $G$.
\end{definition}

\noindent Let $f\colon X\to Y$ be a continuous map between topological spaces. Given a k--simplex $\sigma\colon\Delta^k\to X$ in $X$, $f\circ\sigma$ is a k--simplex in $Y$. This gives rise to a homomorphism \[f_\#\colon C_k(X;G)\to C_k(Y;G):\sum\limits_{i=0}^kn_i\sigma_i\mapsto\sum\limits_{i=0}^kn_if\circ\sigma_i.\]
\begin{definition}
...
Now let $f\colon Y\to Z$ and $g\colon X\to X$ be continuous maps. Given $[c]=[\sum\limits_{i=0}^kn_i\sigma_i]\in H_k(X;G)$, \[(f\circ g)_*([c])=[\sum\limits_{i=0}^kn_if\circ g(\sigma_i)]=f_*([\sum\limits_{i=0}^kn_ig(\sigma_i)])=f_*\circ g_*([c])\] so $(f\circ g)_*=f_*\circ g_*$.
\end{proof}
...
\end{lemma}
\begin{proof}
A singular 0-simplex is a map from a point to a point $x\in X$. let $c=\sum\limits n_i\sigma_i$ be a 0-chain. $\partial c=0$ so $\text{ker}(\partial:C_{0}(x)\rightarrow C_{-1}(X))=C_0(X)$ giving $H_0(X)=\frac{C_0(X)}{\partial C_1(X)}$. Let $x,y\in X$ and let $\sigma_x$ and $\sigma_y$ be their corresponding simplicial 0-simplices. $X$ is path-connected so there exists a simplicial 1-simplex $\sigma$ with $\partial\sigma=\sigma_x-\sigma_y$. Define $f\colon C_0(X)\to\mathbb{Z}:\sum\limits n_ix_i\mapsto\sum\limits n_i$. Clearly $\partial C_1(X)\in \text{ker}f$. Let $z=\sum\limits_{i=0}^ka_i\sigma_i\in\text{ker}f$. Express $a_i\sigma_i$ as $\sum\limits_{j=1}^{|a_i|}a_{ij}\sigma_i$ where $a_{ij}$ is either all $1$ or all $-1$. Thus $z=\sum\limits_{j=1}^rb_j\tau_j$ where $\tau_j\in\{\sigma_0,...,\sigma_k\}$ and $b_j=\pm1$. $|\{j:b_j=1\}|=|\{j:b_j=-1\}|$ so $r$ is even and we can express $z$ as a sum of terms of the form $(\sigma_\alpha - \sigma_\beta)\in\partial C_1(X)$, giving $\text{ker}f\in\partial C_1(X)$. Thus $\text{ker}f\in\partial C_1(X)$. $f$ is surjective so the result follows by the first isomorphism theorem.
\end{proof}
...
Also $\partial_k(n\sigma_k)=\sum\limits_{i=0}^k(-1)^i n\sigma_{k-1}$ which is zero if $k$ is odd and $n\sigma_{k-1}$ if $k$ is even. Thus the induced map on the coefficients of the boundary is $0$ if $k$ is odd and id if $k$ is even. The homology groups will then be $\frac{\mathbb{Z}}{\mathbb{Z}}=0$ or $\frac{\{0\}}{\{0\}}=0$, except for $H_0$ which is $\mathbb{Z}$ since $\{\text{pt}\}$ is path-connected.
\end{proof}

\begin{lemma}
If $\{X_i\}$ is the set of path-components of a topological space $X$, then\[H_k(X;G)\cong\bigoplus_i H_k(X_i;G).\]
\end{lemma}
\begin{proof}
The image of any singular $k$--simplex is contained in a path-component so\[C_k(X;G)\cong\bigoplus_i C_k(X_i;G).\]
\end{proof}
...
$G/[G,G]$ is abelian so the letters in $[w]$ can be reordered such that $[w]=\sum\limits n_i[a_i]=0$ implying $w\in\text{ker }\pi=[G,G]$. 
\end{proof}
...
Relative homology will be used to define the orientation of manifolds in terms of homology.
\begin{definition}
\cite{wiki}Let $X$ be a topological space and let $A$ be a subspace of $X$. The \textbf{$n$'th relative chain group} is \[C_n(X,A):=\frac{C_n(X)}{C_n(A)}.\]
\end{definition}

\noindent The boundary map \[\partial:C_n(X)\to C_{n-1}(X)\] restricts to a boundary map \[\partial_{|C_n(A)}:C_n(A)\to C_{n-1}(A)\] and thus induces a boundary map $\partial'$ on $C_n(X;A)$.
...
\section{Cohomology}
Cohomology is the dual notion to homology -- instead of chains, we have cochains (homomorphisms of chains); and instead of boundaries, we have coboundaries. We shall see that the cohomology groups admit a ring structure, providing them with more content. We shall also study Poincar\'e duality and the universal coefficient theorem, which give relationships between the homology and cohomology groups.


\subsection{Modules}[Introduction to tor \cite{tor}][Differential topology \cite{Nordstrom}]
...
An abelian group $G$ is a $\mathbb{Z}$-module given by $ng=$$\begin{cases}
       \sum\limits_{i=1}^ng &\quad\text{if }n>0 \\
       0 &\quad\text{if }n=0 \\
       -\sum\limits_{i=1}^{-n}g &\quad\text{if }n<0 \\
     \end{cases}$\\
\end{example}

\begin{definition}
Let $A$ and $B$ be R-modules. Their \textbf{direct sum} $A\oplus B$ is the module $A\times B$ under component-wise operations i.e. $(a_1,b_1)+(a_2,b_2)=(a_1+a_2,b_1+b_2)$ and $r(a,b)=(ra,rb)$. This can be extended to any finite collection of R-modules.
\end{definition}
...
\begin{definition}
Let $M$ and $N$ be modules. Let $I$ denote the submodule of $M\times N$ generated by the set of elements of the form $(\lambda m_1+m_2,n)-\lambda(m_1,n)-(m_2,n)$, $(m,\lambda n_1+n_2)-\lambda(m,n_1)-(m,n_2)$ for $m,m_1,m_2\in M$, $n,n_1,n_2\in N$ and $\lambda\in R$. Then the \textbf{tensor product} of $M$ and $N$ is $M\otimes_R N=R^{M\times N}/I$ whose elements are represented as $m\otimes n$ for $m\in M$, $n\in N$.
\end{definition}
...
\begin{example}
If $p$ and $q$ are coprime then $\mathbb{Z}_p\otimes_\mathbb{Z}\mathbb{Z}_q=0$ since there exist $m,n\in\mathbb{Z}$ such that $mp+nq=1$ (Bezout's lemma) so $a\otimes b=(mp+nq)a\otimes b=(mpa)\otimes b + a\otimes(nqb)=0\forall a\otimes b\in\mathbb{Z}_p\otimes_\mathbb{Z}\mathbb{Z}_q$.
\end{example}
\begin{example}
$C_k(X;G)=C_k(X)\otimes_\mathbb{Z}G$.
\end{example}

\begin{example}
$\mathbb{Z}_m\otimes\mathbb{Z}_n=\mathbb{Z}_{gcd(m,n)}$.
\end{example}
\begin{proof}
...
\begin{definition}
\cite{quick}Let $R$ be a ring and $X$ be a topological space. The \textbf{cup product} is the product on cochains\[C^i(X;R)\times C^j(X;R)\to C^{i+j}(X;R):(\psi,\phi)\mapsto\psi\cup\phi\] where \[(\psi\cup\phi)\sigma=\psi(\sigma\circ F_{[e_0,...,e_i]})\phi(\sigma\circ F_{[e_i,...,e_{i+j}]})\] for a singular $i+j$-simplex $\sigma\colon\Delta^{i+j}\to X$ and is extended linearly to $C_{i+j}(X)$.
\end{definition}

\noindent Informally, this means evaluating $\psi$ on the front face of $\sigma$, evaluating $\phi$ on the back face of $\sigma$, and multiplying them together.
...
\begin{align*}
d(\psi\cup\phi)(\sigma)&=\psi\cup\phi(\partial\sigma)\\&=\psi\cup\phi\big(\sum\limits_{t=0}^{j+k+1}(-1)^t\sigma\circ F_{[e_0,...,\overset{\wedge}{e_t},...,e_{i+j+1}]}\big)\\&=\sum\limits_{t=0}^{i+j+1}(-1)^t\psi\cup\phi(\sigma\circ F_{[e_0,...,\overset{\wedge}{e_t},...,e_{i+j+1}]})\\&=\sum\limits_{t=0}^{i}(-1)^t\psi\cup\phi(\sigma\circ F_{[e_0,...,\overset{\wedge}{e_t},...,e_{i+j+1}]})+\sum\limits_{t=i+1}^{i+j+1}(-1)^t\psi\cup\phi(\sigma\circ F_{[e_0,...,\overset{\wedge}{e_t},...,e_{i+j+1}]})\\&=\sum\limits_{t=0}^i(-1)^t\psi(\sigma\circ F_{[e_0,...,\overset{\wedge}{e_t},...,e_{i+1}]})\phi(\sigma\circ F_{[e_{i+1},...,e_{i+j+1}]})\\&+\sum\limits_{t=i+1}^{i+j+1}(-1)^t\psi(\sigma\circ F_{[e_0,...,e_{i}]})\phi(\sigma\circ F_{[e_{i},...,\overset{\wedge}{e_t},...,e_{i+j+1}]})\\
&=\sum\limits_{t=0}^{i+1}(-1)^t\psi(\sigma\circ F_{[e_0,...,\overset{\wedge}{e_t},...,e_{i+1}]})\phi(\sigma\circ F_{[e_{i+1},...,e_{i+j+1}]})\\&+\sum\limits_{t=i}^{i+j+1}(-1)^t\psi(\sigma\circ F_{[e_0,...,e_{i}]})\phi(\sigma\circ F_{[e_{i},...,\overset{\wedge}{e_t},...,e_{i+j+1}]}).
\end{align*}

\noindent We have \[(d\psi\cup\phi)(\sigma)=\sum\limits_{t=0}^{i+1}(-1)^t\psi(\sigma\circ F_{[e_0,...,\overset{\wedge}{e_t},...,e_{i+1}]})\phi(\sigma\circ F_{[e_{i+1},...,e_{i+j+1}]})\]
and \[(-1)^i(\psi\cup d\phi)(\sigma)=\sum\limits_{t=i}^{i+j+1}(-1)^t\psi(\sigma\circ F_{[e_0,...,e_i]})\phi(\sigma\circ F_{[e_i,...,\overset{\wedge}{e_t},...,e_{i+j+1}]})\] When these expressions are added, the last term of the first sum cancels with the first term of the second sum, leaving $d(\psi\cup\phi)(\sigma)$.
\end{proof}
...
\end{proposition}
\begin{proof}
See Hatcher page $210$\cite[210]{Hatcher}.
\end{proof}
...
Poincar\'e duality gives explicit isomorphisms between the homology and cohomology groups. However, we first need to study the cap product, with which the Poincar\'e duality isomorphism is defined.
\begin{definition}
...
\begin{proof}
We have
\[\partial\sigma\cap\phi=\sum\limits_{t=0}^i(-1)^t\phi(\sigma\circ F_{[e_0,...,\overset{\wedge}{e_t},..,e_{i+1}]})\sigma\circ F_{[e_{i+1},...,e_{i+j}]}+\sum\limits_{t=i+1}^{i+j}(-1)^t\phi(\sigma\circ F_{[e_0,..,e_{i}]})\sigma\circ F_{[e_{i},...,\overset{\wedge}{e_t},...,e_{i+j}]},\]
\[\sigma\cap d\phi=\sum\limits_{t=0}^{i+1}(-1)^t\phi(\sigma\circ F_{[e_0,...,\overset{\wedge}{e_t},..,e_{i+1}]})\sigma\circ F_{[e_{i+1},...,e_{i+j}]},\]
\[\partial(\sigma\cap\phi)=\sum\limits_{t=i}^{i+j}(-1)^{t-i}\phi(\sigma\circ F_{[e_0,..,e_{i}]})\sigma\circ F_{[e_{i},...,\overset{\wedge}{e_t},...,e_{i+j}]}.\] Thus \begin{align*}\text{RHS}&=(-1)^i((-1)^{i+1+1}\phi(\sigma\circ F_{[e_0,...,e_i]})\sigma\circ F_{[e_{i+1},...,e_{i+j}]}+\sum\limits_{t=i+1}^{i+j}(-1)^t\phi(\sigma\circ F_{[e_0,..,e_{i}]})\sigma\circ F_{[e_{i},...,\overset{\wedge}{e_t},...,e_{i+j}]})\\&=(-1)^i(\sum\limits_{t=i}^{i+j}(-1)^t\phi(\sigma\circ F_{[e_0,..,e_{i}]})\sigma\circ F_{[e_{i},...,\overset{\wedge}{e_t},...,e_{i+j}]})\\&=\sum\limits_{t=i}^{i+j}(-1)^{t-i}\phi(\sigma\circ F_{[e_0,..,e_{i}]})\sigma\circ F_{[e_{i},...,\overset{\wedge}{e_t},...,e_{i+j}]}=\partial(\sigma\cap\phi).\end{align*}\qedhere
\end{proof}
...
\begin{proposition}
Let $f\colon X\to Y$ be continuous, $c\in C_{i+j}(X;R)$ and $\phi\in C^i(Y;R)$. Then \[f_*(c)\cap\phi=f_*(c\cap f^*(\phi)).\]
\end{proposition}
\begin{proof}
...
\cite[Differential topology]{Nordstrom}
...
Manifolds are spaces locally homeomorphic to $\mathbb{R}^n$ and allow for additional topological invariants to be defined on them. We will first see that manifolds admit a concept of orientation, as a consequence of their $n$'th local homology groups being isomorphic to $\mathbb{Z}$, which allows for isomorphisms between the homology and cohomology groups to be defined (Poincar\'e duality). We will also see that certain types of manifolds admit a topological invariant known as the torsion linking form, which will be used for the homotopy classification of lens spaces.
\subsection{Orientation of manifolds}
We have already seen formulations of the concept of orientation in Differential and geometric analysis \cite{dga} through orientation forms or consistent choices of unit normals on submanifolds. However, here we shall present the homological definition, and see a result that will allow us to infer the orientability of lens spaces from the orientability of $S^3$.
\begin{definition}
...
\end{definition}
\begin{lemma}
\cite{wiki}Let $M$ be an $n$--manifold, let $x\in M$ and let $U$ be a neighbourhood of $x$ homeomorphic to $\mathbb{R}^n$. Then \[H_n(M,M-x)\cong\mathbb{Z}.\]
\end{lemma}
\begin{proof}
...
\begin{remark}
The homological definitions of orientation and orientation-preserving/reversing homeomorphisms are consistent with those from MA40254 \cite{dga}for submanifolds in terms of differential forms.
\end{remark}

\begin{lemma}[A short survey of lens spaces{\cite[1.1.6]{Lens}}]
If $X$ is a compact Hausdorff space acted on by a finite group $G$ by homeomorphisms, then $X/G$ is compact Hausdorff.
\end{lemma}
\begin{proof}
...
\end{lemma}
\begin{proof}
Let $M$ be an $n$--manifold and let $U\subseteq M$ be open. Let $x\in U$. Then there exists an open neighbourhood $S\subseteq M$ of $x$ and a homeomorphism $f:S\to V$ where $V\subseteq\mathbb{R}^n$ is open. $S\cap U$ is then also an open neighbourhood of $x$ and $f_{|U}U\to f(U)$ is also a homeomorphism between open sets, since the restriction of a homeomorphism is a homeomorphism and homeomorphisms map open sets to open sets. $U$ is also Hausdorff as a subspace of a Hausdorff space so $U$ is an $n$--manifold.
\end{proof}
...
\noindent Having studied the cap product, we are now ready to state Poincar\'e duality. However, Poincar\'e duality doesn't hold for arbitrary topological spaces, but for closed oriented manifolds. We shall now define and study those terms.
...
\begin{theorem}
Any compact oriented manifold has a fundamental class.
\end{theorem}
...
\begin{proposition}
Let $M$ be a closed connected n-dimensional manifold. Then $H_n(M)\cong \mathbb{Z}$ if $M$ is orientable. Furthermore, $H_i(M)=0$ for $i>n$.
\end{proposition}
...
\noindent We then also have an alternate definition for orientation-preserving and orientation-reversing homeomorphisms:
\begin{definition}
...
\noindent We can now state the Poincar\'e Duality theorem as an explicit isomorphism.

\begin{theorem}
(Poincar\'e Duality)\cite{Hatcher}. If $M$ is a closed $R$-orientable $n$-manifold with fundamental class $[M]\in H_n(M;R)$, then the map $D\colon H^k(M;R)\to H_{n-k}(M;R)$ defined by $D([\alpha])=[M]\cap[\alpha]$ is an isomorphism for all $k$.
\end{theorem}

\subsection{Torsion linking form}
The torsion linking form is a homotopy invariant for closed oriented manifolds. We shall define it and prove basic results about it here.
\begin{definition}
Let $M^n$ be a closed oriented manifold with fundamental class $[M]\in H_n(M;\mathbb{Z})$. Let $[\alpha]\in H^a(M;\mathbb{Z})$ and $[\beta]\in H^{n-a+1}(M;\mathbb{Z})$ be torsion classes, i.e. $k[\alpha]=0,m[\beta]=0$ for some $k,m\in\mathbb{N}$. Then we can write $k\alpha=d\gamma$ for some $\gamma\in C^{a-1}(M,\mathbb{Z})$. Let $A$ be a cycle representing the fundamental class $[M]$. We define $[\alpha]\vee[\beta]=\frac{1}{k}(\gamma\cup\beta)A\in\mathbb{Q}/\mathbb{Z}$. The \textbf{torsion linking pairing} is then \[T(H^a(M;\mathbb{Z}))\times T(H^{n-a+1}(M;\mathbb{Z}))\to\mathbb{Q}/\mathbb{Z}:([\alpha],[\beta])\mapsto[\alpha]\vee[\beta].\]
\end{definition}
...
\noindent Finally, let $[\beta]=[\beta']\in H^{n-a+1}(M;\mathbb{Z})$. Then $\beta'=\beta+df$ for some $f\in C^{n-a}(M,\mathbb{Z})$. \[d(\gamma\cup f)=k(\alpha\cup f)+(-1)^{a-1}(\gamma\cup df)\] so \[\frac{1}{k}(\gamma\cup df)A=\frac{(-1)^{1-a}}{k}d(\gamma\cup f)A=\frac{(-1)^{1-a}}{k}([0],[M])=0.\] Thus \[\frac{1}{k}(\gamma\cup\beta')A=\frac{1}{k}(\gamma\cup(\beta+df))A=\frac{1}{k}(\gamma\cup\beta)A+\frac{1}{k}(\gamma\cup df)A=\frac{1}{k}(\gamma\cup\beta)A,\] implying independence of $\beta$. Thus the pairing is well-defined.
\end{proof}
...
\end{lemma}
\begin{proof}
Let $m\beta=d\tau$ for some $\tau\in C^{n-a}(M,\mathbb{Z})$. If we consider $\tau$ and $\alpha$ to be representatives of elements of $H^*(M,\mathbb{Z}_m)$, we have graded commutativity. Thus 
\begin{align*}
[\beta]\vee[\alpha]&=\frac{1}{m}(\tau\cup\alpha)A\\
&=\frac{1}{m}([\tau]\cup[\alpha],[M])\\&=(-1)^{a(n-a)}\frac{1}{m}([\alpha]\cup[\tau],[M])\\&=(-1)^{a(n-a)}\frac{1}{m}(\alpha\cup\tau)A\\&=\frac{(-1)^{a(n-a)}}{km}(k\alpha\cup\tau)A\\&=\frac{(-1)^{a(n-a)}}{km}(d\gamma\cup\tau)A\in\mathbb{Q}/\mathbb{Z}.\end{align*} Furthermore,\[d(\gamma\cup\tau)=(d\gamma\cup\tau)+(-1)^{a-1}(\gamma\cup d\tau)=(d\gamma\cup\tau)+(-1)^{a-1}(\gamma\cup m\beta)\] so \begin{align*}[\beta]\vee[\alpha]=\frac{(-1)^{a(n-a)}}{km}(d\gamma\cup\tau)A&=\frac{(-1)^{a(n-a)}}{km}(d(\gamma\cup\tau)A-(-1)^{a-1}(\gamma\cup m\beta)A)\\&=\frac{(-1)^{a(n-a)}}{km}(-1)^a(\gamma\cup m\beta)A\\&=\frac{(-1)^{a(n-a+1)}}{k}(\gamma\cup\beta)A\\&=(-1)^{a(n-a+1)}[\alpha]\vee[\beta]\qedhere\end{align*}
\end{proof}
...
\section{Classification of lens spaces}
In this section we shall apply the theory developed erstwhile and introduce additional theory to obtain a classification of lens spaces up to homeomorphism and homotopy equivalence. We shall first compute the basic topological invariants: the fundamental group, the homology groups and the cohomology groups. These invarients are insufficient for the classification, so we shall also compute the stronger invariants of the torsion linking form and Reidemeister torsion.\\

\noindent The torsion linking form requires explicit descriptions of cochains, for which singular homology, having uncountably infinitely many chains on which cochains need to be evaluated, is intractable. We thus introduce delta complexes: geometric realisations of topological spaces which, in the case of lens spaces, admit chain groups with finitely many generators, making them amenable to explicitly describe cochains and compute cup products thereon.\\

\noindent A delta complex is a cell complex with additional requirements. This will make a mere cell complex structure more useful for the Reidemeister torsion computation, since it will be seen that the chain groups can admit a single generator, making the computation simpler. However, the extra rigidity of delta complexes allows for cup products to be computed, which will be necessary for the torsion linking form. 

\subsection{Lens spaces}
Here we shall define lens spaces and derive the basic results about them. In particular, we shall prove that they are closed orientable $3$--manifolds and compute their fundamental group, homology groups and cohomology groups.
\begin{definition}
...
\end{proof}
\begin{remark}
Alternatively, we could note that $\phi$ is homotopic to the identity and thus induces the identity map on homology so maps a fundamental class to itself so is orientation-preserving.
\end{remark}
...
We shall now define cell complexes, and their special case of delta complexes.
\begin{definition}
...
\begin{definition}
Let $e_i^k$ be a $k$-cell with an attaching map $\phi_i\colon S^{k-1}\to X^{k-1}$. Given a $(k-1)$-cell $e_j^{k-1}$ we have a map $\phi_i^j:\phi_i^{-1}(e_j^{k-1})\to e_j^{k-1}:x\mapsto\phi_i(x)$. Then We define the boundary map $\partial\colon CC_k(X)\to CC_{k-1(X)}$ by $\partial e_i^k=\sum\limits_jdeg(\phi_i^j)e_j^{k-1}$ and extending linearly.
\end{definition}
...
\begin{proposition}
\cite{Nordstrom}Let $G$ be an abelian group. Then\[H_k(CC_k(X;G),\partial)\cong H_k(X;G)\] and \[H^k(CC^k(X;G),d)\cong H^k(X;G)\] for all $k\in\mathbb{N}$.
\end{proposition}
...
\begin{figure} [hbt!]
    \centering
    \includegraphics[width=1\linewidth]{Screenshot 2024-04-17 at 14-48-00 lensspaces.pdf.png}
\end{figure}
...
\noindent This then gives a model of $L(p;q)$, which we shall now use to obtain a delta complex.\\
...
\noindent Points: $[N]$ and $[x_0]$ (since $\{x_0,x_q,x_{2q},...\}$ are identified and $q$ is coprime to $p$)\\
...
\noindent Having obtained a delta-complex structure, we are now able to compute the torsion linking form of a lens space:
...
\[\text{Let }E=[NS].\]
\[\text{Let }F_i=[Nx_i].\]
\begin{lemma}$\partial b_i=D-F_{i+1}+F_i$.
\end{lemma}
\begin{proof}
...
\begin{lemma}
$\sum\limits_{i=0}^{p-1}a_i$ is a representative of a fundamental class.
\end{lemma}
\begin{proof}
\[\partial(\sum\limits_{i=0}^{p-1}a_i)=\sum\limits_i([Nx_{i-q}x_{i-q+1}]-[Nx_ix_{i+1}]+[NSx_{i+1}]-[NSx_i])=0\] so $\sum\limits_{i=0}^{p-1}a_i$ is a representative of an element of $H_3(L(p;q);\mathbb{Z})$. The coefficients of $a_i$ are all equal and $H_3(L(p;q);\mathbb{Z})\cong\mathbb{Z}$ so $\sum\limits_{i=0}^{p-1}a_i$ has to represent a generator, since $\partial(\sum\limits_{i=0}^{p-1}\lambda_i a_i)=0$ if and only if all $\lambda_i$ are equal. Thus $\sum\limits_{i=0}^{p-1}a_i$ represents a fundamental class.
\end{proof}

\noindent Call the equivalence class it represents $[M]$.\\
...
\[d\widehat D=\sum\limits_{i=0}^{p-1}\widehat b_i.\]
\[d\widehat E=\sum\limits_{i=0}^{p-1}\widehat c_i.\]
\[d\widehat F_i=\widehat b_i-\widehat b_{i-1}-\widehat c_i+\widehat c_{i+q}.\]
Then define \[\gamma=\widehat D + (\sum\limits_{i=0}^{p-1}i\widehat F_i)+q\widehat E\in C^1(L(p;q);\mathbb{Z}).\]
\begin{align*}
d\gamma&=\sum\limits_{i=0}^{p-1}\widehat b_i+(\sum\limits_{i=0}^{p-1}i(\widehat b_i-\widehat b_{i-1}-\widehat c_i+\widehat c_{i+q}))+\sum\limits_{i=0}^{p-1}q\hat c_i\\
&=\sum\limits_{i=0}^{p-1}((i+1)\widehat b_i-i\widehat b_{i-1}-(i-q)\widehat c_i+i\widehat c_{i+q})\equiv 0\text{ mod }p.
\end{align*}
...
Otherwise, \[p\alpha([Sx_ix_{i+1}])=p\alpha([Nx_{i-q}x_{i+1-q}])=p\alpha(b_{i-q})=(kp+i-q+1)\hat b_{{i-q}}(b_{{i-q}})-(kp+i-q+1)\hat b_{{i-q}}(b_{{i-q}})=0\]where $k\in\mathbb{Z}$ puts $kp+i-q+1$ in the compatible range for the sum of $[1,p-1]$ (The fact that it can't wrap around to $0$ is why cancellation occurs). Thus, \[[\alpha]\vee[\alpha]=\frac{1}{p}\gamma([NS])\alpha([Sx_{q-1}x_q])=\frac{q}{p}.\]\end{proof}

\noindent We then have that if $L(p;q_1)$ and $L(p;q_2)$ are homotopy-equivalent, then $\frac{q_1}{p}=\pm\frac{q_2}{p}m^2$ for some integer $m$, taking into account whether or not the homotopy-equivalence is orientation-reversing and choice of element of $H^2(L(p;q);\mathbb{Z})$ (since $[n\alpha]\vee[n\alpha]=n^2[\alpha]\vee[\alpha]$). This is equivalent to \begin{align*}
q_1\equiv\pm q_2m^2\text{ mod }p&\iff q_1q_2^{-1}\equiv\pm m^2\text{ mod }p\\&\iff q_1q_2\equiv\pm(mq_2)^2\text{ mod } p\\&\iff q_2q_2\equiv\pm n^2\text{ mod } p\text{, where }n=mq_2.\end{align*}\\
...
Thus $L(p;q)$ is orientation-reversing homotopy-equivalent to itself iff $q(-q)$ is a square mod $p$ $\iff -1$ is a square mod $p\iff p\equiv 1$ mod $4$ for $p$ a prime. This in contrast to compact $2$--manifolds, which by the classification of compact surfaces\cite{intro} always admit orientation-reversing homeomorphisms.\\

\noindent The proof of the sufficiency of this condition is given in exercise 29 in section 4.2 of Hatcher. \cite{Hatcher}



\subsection{Reidemeister torsion}[Notes on Reidemeister torsion\cite{torsion}][Pierre ALbin\cite{lecture}]
We shall now define the invariant which will give us the homeomorphism classification.
\begin{definition}
...
\noindent Let \[X=\bigcup_{k=0}^n\bigcup_je_j^k\] be a finite cell complex and $(A,\beta)$ a representation of $\pi_1(X)$ in a commutative ring $A$; that is, we have a homomorphism $\beta:\pi_1(X)\to A^*$. Let $\overset{\sim}{X}$ be a universal cover of $X$ and let $\overset{\sim}{e}_j^k$ be a lift of $e_j^k$. Then every other lift of $e_j^k$ is obtained by composition of $\overset{\sim}{e}_j^k$ with a deck transformation (since given $f'$ and $g'$ lifts of $f$ there exists a $\phi\in\text{Aut}(p)$ such that $\phi\circ f'(0)=g'(0)$ which implies $\phi\circ f=g$ by uniqueness\cite{alg}). $\text{Aut}(p)\cong\pi_1(X)$ so we can write \[\overset{\sim}{X}=\bigcup_{k=0}^n\bigcup_{g\in\pi_1(X)}\bigcup_jg\overset{\sim}{e}_j^k.\] Thus multiplication by an element of $\pi_1(X)$ permutes the cells, thereby giving $CC_.(\overset{\sim}{X})$ a $\pi_1(X)$-action. $A$ also has a $\pi_1(X)$ action so we can define $C_k(X;\beta)=CC_k(\overset{\sim}{X})\otimes_{\mathbb{Z}\pi_1(X)} A$, which is the quotient of $CC_k(\overset{\sim}{X})\otimes A$ by the action of $\pi_1(X)$. We have an induced boundary map $\partial\colon C_k(X;\beta)\to C_{k-1}(X;\beta)$ which makes $C_k(X;\beta)$ a chain complex with basis $\{\overset{\sim}{e}_j^k\}\otimes\text{id}_A$.
...
\noindent Thus the Reidemeister torsion is invariant under homeomorphism. However, it is not invariant under homotopy-equivalence.

\subsection{Homeomorphism classification}[Pierre Albin\cite{lecture}]
...
\[\partial e_0^1=e_l^0-e_0^0=(\rho^l-\text{id})(e_0^0),\] where $lq\cong1\text{ mod }p$.
...
\noindent The following lemma then gives the necessary conditions for the homeomorphism classification.

\begin{lemma}
Given $q_1$, $q_2$ and a primitive $p$th root of unity $\omega$, there is another primitive root of unity $\nu$ such that $\Delta(L(p;q_1),\beta_\omega)=\Delta(L(p;q_2),\beta_\nu)$ only if $q_1\equiv\pm q_2\text{ mod }p$ or $q_1q_2\equiv\pm 1\text{ mod }p$
\end{lemma}

\noindent To prove the sufficiency of the above lemma for the homeomorphism classification, we require the following lemma.
