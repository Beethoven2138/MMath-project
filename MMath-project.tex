...
$\gamma(e)=q$.\\
$p\alpha([Sx_{p-1}x_0])=-p\alpha(b_{p-1})=-((p-1+1)\delta_{B_{p-1}}(-B_{p-1})-(0)\delta_{B_{p-1}}(-B_{p-1}))=p\implies \alpha([Sx_{p-1}x_0])=1$.\\
Otherwise, $p\alpha([Sx_ix_{i+1}])=p\alpha(b_i)=(i+1)\delta_{B_{i}}-(i+1)\delta_{B_{i}}=0$.\\
Thus $[\alpha]\vee[\alpha]=\frac{1}{p}\gamma([NS])\alpha([Sx_{p-1}x_0])=\frac{q}{p}$.\\

\noindent We then have that if $L(p;q_1)$ and $L(p;q_2)$ are homotopy-equivalent, then $\frac{q_1}{p}=\pm\frac{q_2}{p}m^2$ for some integer $m$, taking into account choice of fundamental class and choice of generator of $H^2(L(p;q);\mathbb{Z})$. This is equivalent to $q_1\equiv\pm q_2m^2\text{ mod }p\iff q_1q_2^{-1}\equiv\pm m^2\text{ mod }p\iff q_1q_2\equiv\pm(mq_2)^2\text{ mod } p\iff q_2q_2\equiv\pm n^2\text{ mod } p$, where $n=mq_2$.
