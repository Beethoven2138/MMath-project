...
Now let $f\colon X\to Y$ and $g\colon Y\to Z$ be continuous maps. $(g\circ f)^*[A](c)=A((g\circ f)_\#(c))=A(g_\#\circ f_\#(c))=(A\circ g_\#)(f_\#(c))=f^\#(A\circ g_\#)(c)=f^\#\circ g^\#A(c)=[f^\#\circ g^\# A](c)=f^*[g^\#A](c)=(f^*\circ g^*)[A](c)\forall c\in C_k(X)$ so $(g\circ f)^*=f^*\circ g^*$.
\end{proof}
