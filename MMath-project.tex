...
\noindent Let $r\in CC^2(L(p;q);\mathbb{Z})$ be given by $r(e^2)=p$. Then define $s\in CC^1(L(p;q);\mathbb{Z})$ by $s(e^1)=1$. $ds(e^2)=s(\partial e^2)=s(pe^1)=p$ so $ds=r$ making $r$ a coboundary. Thus $H^2(L(p;q);\mathbb{Z})$ is represented by $e^2\mapsto 0,...,p-1$. So let $\alpha=(e^2\mapsto q)$ be a representative of a generator. $p\alpha=qr$ so let $\gamma=(e^1\mapsto q)$. Then $\frac{1}{p}(\gamma\cup\alpha)[M]=\frac{q^2}{p}$?
...
\noindent Thus homotopy-equivalent closed manifolds are not necessarily homeomorphic.\\
...
$\partial[N]=\partial[x_0]=0$.\\
$\partial[NS]=0$, $\partial[x_0x_1]=[x_1]-[x_0]=0$, $\partial[Nx_i]=[x_i]-[N]$.\\
...
Let $e=[NS]$.\\
Let $f_i=[Nx_i]$.\\
$\partial((\sum_{i=0}^{p-2}a_i)-a_{p-1})=\sum_i([Nx_{i-q}x_{i-q+1}]-[Nx_ix_{i+1}]+[NSx_{i+1}]-[NSx_i])=0$ so $(\sum_{i=0}^{p-2}a_i)-a_{p-1}$ is a representative of an element of $H_3(L(p;q);\mathbb{Z})$. In fact, it is a fundamental class. Call the equivalence class it represents $[M]$.\\
We also have $\partial b_i=d-f_{i+1}+f_i$ and $\partial c_i=e-f_i+f_{i-q}$ for $i\neq p-1$ and $\partial b_{p-1}=-d+f_{0}-f_{p-1}$, $\partial c_{p-1}=-e+f_{p-1}-f_{p-1-q}$ indexed modulo $p$.\\
If we define $B_i$ by $B_{p-1}=-b_{p-1}$ and $B_i=b_i$ otherwise and $C_i$ by $C_{p-1}=-c_{p-1}$ and $C_i=c_i$ otherwise, then we have $\partial B_i=d-f_{i+1}+f_i$ and $\partial C_i=e-f_i+f_{i-q}$.\\
This gives:\\
$d\delta_d=\sum_iB_i$,\\
$d\delta_e=\sum_iC_i$,\\
...
$\gamma(e)=q$.\\
$p\alpha([Sx_{p-1}x_0])=-p\alpha(b_{p-1})=-((p-1+1)\delta_{B_{p-1}}(-B_{p-1})-(-1)\delta_{B_{p-1}}(-B_{p-1}))=-(-p-1)=p+1$.\\
Otherwise, $p\alpha([Sx_ix_{i+1}])=p\alpha(b_i)=(i+1)\delta_{B_{i}}-(i+1)\delta_{B_{i}}=0$.\\
Thus $[\alpha]\vee[\alpha]=\frac{q(p+1)}{p}=\frac{q}{p}$.\\
a\\
a\\
a\\
a\\
...
a\\
Lines: $[NS]$, $[x_0x_1]$ and $[Nx_0],...,[Nx_{p-1}]$\\
Faces: $[Nx_0x_1],...,[Nx_{p-1}x_0]$ and $[Nx_0S],...,[Nx_{p-1}S]$\\
Slices: $[Nx_0Sx_1],...,[Nx_{p-1}Sx_0]$\\
Note: $[Nx_i]=-[x_{i+q}S]$\\

\noindent $\partial[N]=\partial[x_0]=0$.\\
$\partial[NS]=0$, $\partial[x_0x_1]=[x_1]-[x_0]=0$, $\partial[Nx_i]=[x_i]-[N]$.\\
$\partial[Nx_ix_{i+1}]=[x_ix_{i+1}]-[Nx_{i+1}]+[Nx_i]=[x_0x_1]-[Nx_{i+1}]+[Nx_i]$.\\
$\partial[Nx_iS]=[x_iS]-[NS]+[Nx_i]=-[Nx_{i-q}]-[NS]+[Nx_i]$.\\
$\partial[Nx_iSx_{i+1}]=[x_iSx_{i+1}]-[NSx_{i+1}]+[Nx_ix_{i+1}]-[Nx_iS]$\\
$=-[Nx_{i-q}x_{i+1-q}]+[Nx_{i+1}S]+[Nx_ix_{i+1}]-[Nx_iS]$.

\noindent $\partial\sum_i[Nx_iSx_{i+1}]=\sum_i([Nx_ix_{i+1}]-[Nx_{i-q}x_{i+1-q}])+\sum_i([Nx_{i+1}S]-[Nx_iS])=0$ so $H_3(L(p;q);\mathbb{Z})$ is generated by $\sum_i[Nx_iSx_{i+1}]$.\\
$\partial\sum_{i=0}^{q-1}[Nx_ix_{i+1}]=q[x_0x_1]+[Nx_0]-[Nx_q]$\\
So $\partial((\sum_{i=0}^{q-1}[Nx_ix_{i+1}])+[Nx_qS])=q[x_0x_1]+[Nx_0]-[Nx_q]-[Nx_0]-[NS]+[Nx_q]=q[x_0x_1]-[NS]$ meaning $[[NS]]=q[[x_0x_1]]$.\\
\noindent $\partial \sum_i[Nx_ix_{i+1}]=p[x_0x_1]+\sum_i(-[Nx_{i+1}]+[Nx_i])=p[x_0x_1]$ so $[[x_0x_1]]$ is a generator of $H_1(L(p;q);\mathbb{Z})$.\\
Let $\alpha\in C^2(L(p;q);\mathbb{Z})$ be given by $\alpha([Nx_ix_{i+1}])=-i$, $\alpha([Nx_iS])=iq$.
Then $d\alpha([Nx_iSx_{i+1}])=-(q-i)+(i+1)q-i-iq=0$ so $\alpha$ is a representative of an element of $H^2(L(p;q);\mathbb{Z})$.\\
Let $\gamma\in C^1(L(p;q);\mathbb{Z})$ be such that $d\gamma=r\alpha$.\\
$r\alpha(\sum_i[Nx_ix_{i+1}])=p\gamma([x_0x_1])=r\sum_i-i=-r\frac{p(p-1)}{2}$ so $\gamma([x_0x_1])=\frac{r(1-p)}{2}$.\\
$r\alpha((\sum_{i=0}^{q-1}[Nx_ix_{i+1}])+[Nx_qS])=q\gamma([x_0x_1])-\gamma([NS])=\frac{qr(1-p)}{2}-\gamma([NS])=r(\frac{q(1-q)}{2}+q^2)\implies \gamma([NS])=r(\frac{q(1-p)-q(1-q)-2q^2}{2})=-r(\frac{pq+q^2}{2})=-rq(\frac{p+q}{2})$.\\
$r\alpha([Nx_ix_{i+1}])=-ri=\frac{r(1-p)}{2}-\gamma([Nx_{i+1}])+\gamma([Nx_i])$ has solution $\gamma([Nx_i])=\frac{ir(i-p)}{2}$.\\
$r\alpha([Nx_iS])=riq=\gamma([Nx_i])-\gamma([Nx_{i-q}])+rq(\frac{p+q}{2})$\\
...
$\partial(\overset{\sim}{e}_0^2\otimes\text{id})=(1+\omega+\ometa^2+...+\omega^{p-1})(\overset{\sim}{e}_0^1\otimes\text{id})$\\
