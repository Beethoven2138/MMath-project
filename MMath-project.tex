...
\end{bmatrix}\]
This is a block matrix so the determinant is the product of the two blocks, giving \[\text{det}(\Phi)=(\text{cos}^2(2\pi/p)+\text{sin}^2(2\pi/p))(\text{cos}^2(2\pi q/p)+\text{sin}^2(2\pi q/p))=1*1=1.\] Thus $(\phi^*\omega)_p=\omega_p$ and so $\phi$ is orientation-preserving. $\phi$ is the action of a generator of $\mathbb{Z}_p$ and the composition of orientation-preserving homeomorphisms is orientation preserving, hence every element of $\mathbb{Z}_p$ acts by an orientation-preserving homeomorphism.
\end{proof}
\noindent Now that we have shown that $L(p;q)$ is a closed orientable $3$--manifold, we are able to compute all the homology and cohomology groups of $L(p;q)$:\\
