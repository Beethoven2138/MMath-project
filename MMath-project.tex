...
\noindent $(\gamma\cup\beta)[M]=\gamma(\beta\cap M)$. By Poincare duality, $H^{n-a+1}(M;\mathbb{Z})\to H_{a-1}(M;\mathbb{Z}):[\beta]\mapsto[\beta]\cap[M]$ is an isomorphism so $[\beta]\mapsto([\alpha]\mapsto\frac{1}{k}\gamma(\beta\cap M)=\frac{1}{k}(\gamma\cup\beta)[M])$ is as well. Thus the bilinear form is non-degenerate.
\end{proof}
\noindent So in the case of 3-manifolds like lens spaces, the torsion linking pairing is a symmetric, non-degenerate bilinear form on $T(H^2(M;\mathbb{Z}))$.
