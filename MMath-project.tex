...
\begin{definition}
Let $X=\cup_{k=0}^n\cup_je_j^k$ be a finite cell complex and $(A,\rho)$ a representation of $\pi_1(X)$ in a ring $A$. Let $\overset{\sim}{X}$ be a universal cover. And $\overset{\sim}{e}_j^k$ be a lift of $e_j^k$. Then every other lift of $e_j^k$ is obtained by composition of $\overset{\sim}{e}_j^k$ with a deck transformation (since given $f'$ and $g'$ lifts of $f$ there exists a $\phi\in\text{Aut}(p)$ such that $\phi\circ f'(1)=g'(1)$ which implies $\phi\circ f=g$ by uniqueness (lemma 6.9)???).
\end{definition}
