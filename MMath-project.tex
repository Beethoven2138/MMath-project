...
\begin{definition}
Given modules M and N, we define the module $R^{M\times N}$ to be $\{\sum_{i=0}^kn_i(m_i,n_i):n_i\in R,(m_i,n_i)\in M\times N\}$.
\end{definition}
...
\begin{definition}
Let $R$ be a ring and $X$ be a topological space. The \textbf{cup product} is the product on cochains $C^i(X;R)\times C^j(X;R)\to C^{i+j}(X;R):(\psi,\phi)\mapsto\psi\cup\phi$ where $(\psi\cup\phi)\sigma=\psi(\sigma\circ F_{[e_0,...,e_i]})\phi(\sigma\circ F_{[e_{i+1},...,e_{i+j}]})$ for a singular $i+j$-simplex $\sigma\colon\Delta^{i+j}\to X$ and is extended linearly to $C_{i+j}(X)$.
\end{definition}
...
$d(\psi\cup\phi)\sigma=(\psi\cup\phi)(\partial\sigma)=\psi(\partial\sigma\circ F_{[e_0,...,e_i]})\phi(\partial\sigma\circ F_{[e_{i+1},...,e_{i+j+1}]})=\psi(\sum_{t=0}^i(-1)^t\sigma\circ F_{[e_0,...,\overset{\wedge}{e_t},...,e_i]})\phi(\sum_{t=i+1}^{i+j+1}(-1)^t\sigma\circ F_{[e_{i+1},...,\overset{\wedge}{e_t},...,e_{i+j+1}]})=(\sum_{t=0}^i(-1)^t\psi(\sigma\circ F_{[e_0,...,\overset{\wedge}{e_t},...,e_i]}))(\sum_{t=i+1}^{i+j+1}(-1)^t\phi(\sigma\circ F_{[e_{i+1},...,\overset{\wedge}{e_t},...,e_{i+j+1}]}))$
...
Let $[\omega]=[\psi]$ so that $\omega=\psi+d\theta$ for some $\theta\in C^{k-1}(X;R)$. Then 
\end{proof}
