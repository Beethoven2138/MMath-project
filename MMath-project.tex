...
Let $\alpha\in C^2(L(p;q);\mathbb{Z})$ be given by $\alpha([Nx_ix_{i+1}])=\alpha([Nx_iS])=r$.\\
Then $d\alpha([Nx_iSx_{i+1}])=0$ so $\alpha$ is a representative of an element of $H^2(L(p;q);\mathbb{Z})$.\\
...
$r\alpha([Nx_ix_{i+1}])=-ri=\frac{r(1-p)}{2}-\gamma([Nx_{i+1}])+\gamma([Nx_i])$ has solution $\gamma([Nx_i])=\frac{i(r(1-p)+r(i-1))}{2}+c$\\
$r\alpha([Nx_iS])=iq=\gamma([Nx_i])-\gamma([Nx_{i-q}])+rq(\frac{p+q}{2})$\\
