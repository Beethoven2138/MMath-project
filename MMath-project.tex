...
\begin{abstract}
\noindent I would like to thank Johannes Nordstrom for the considerable help and support he provided me throughout this project, and for being an excellent lecturer in last semester's algebraic topology module.
\end{abstract}
...
\noindent Note that the boundary map \[\partial:C_n(X)\to C_{n-1}(X)\] restricts to a boundary map \[\partial_{|C_n(A)}C_n(A)\to C_{n-1}(A)\] and thus induces a boundary map $\partial'$ on $C_n(X;A)$.
...
\begin{theorem}
(Excision theorem) Let $U\subseteq A\subseteq X$ be topological spaces. If the closure of $U$ is contained in the interior of $A$, then $U$ can be excised.\cite{Hatcher}
\end{theorem}
...
\begin{definition}
\cite{tor}Let $R$ be a commutative ring with a unit. An \textbf{R-module} $M$ is an abelian group with a map $f\colon R\times M\to M$ such that if $f(r,m)$ is denoted by $rm$ then $(r_1r_2)m=r_1(r_2m)$ and $1m=m$. We usually abbreviate R-modules to modules.
\end{definition}
\begin{remark}
...
Informally, this means evaluating $\psi$ on the front face of $\sigma$, evaluating $\phi$ on the back face of $\sigma$, and multiplying them together.
...
Manifolds contain additional topological information and allow for additional topological invariants to be defined on them. We will first see that manifolds admit a concept of orientation, as a consequence of their nth local homology groups being isomorphic to $\mathbb{Z}$, which allows for isomorphisms between the homology and cohomology groups to be defined (Poincare duality). We will also see that certain types of manifolds admit a topological invariant known as the torsion linking form, which will be used for the homotopy classification of lens spaces.
\subsection{Orientation of manifolds}
Orientation roughly refers to a consistent choice of coordinate system at each point on a manifold. We have already seen formulations of the concept in Differential and geometric analysis \cite{dga} through orientation forms or consistent choices of unit normal on submanifolds. However, here we shall present the homological definition, and see a result that will allow us to infer the orientability of lens spaces from the orientability of $S^3$.
\begin{definition}
...
\noindent We can now state the poincare Duality theorem as an explicit isomorphism.

\begin{theorem}
(Poincare Duality)\cite{Hatcher}. If $M$ is a closed $R$-orientable $n$-manifold with fundamental class $[M]\in H_n(M;R)$, then the map $D\colon H^k(M;R)\to H_{n-k}(M;R)$ defined by $D([\alpha])=[M]\cap[\alpha]$ is an isomorphism for all $k$.
\end{theorem}

\subsection{Torsion linking form}
The torsion linking form is a homotopy invariant for closed oriented manifolds. We shall state it and prove basic results about it here.
\begin{definition}
...
Let $A$ be a cycle representing the fundamental class $[M]$.
...
Otherwise, \[p\alpha([Sx_ix_{i+1}])=p\alpha([Nx_{i-q}x_{i+1-q}])=p\alpha(b_{i-q})=(kp+i-q+1)\hat b_{{i-q}}(b_{{i-q}})-(kp+i-q+1)\hat b_{{i-q}}(b_{{i-q}})=0\]where $k\in\mathbb{Z}$ puts $kp+i-q+1$ in the compatible range for the sum of $[1,p-1]$ (The fact that it can't wrap around to $0$ is why cancellation occurs). Thus, \begin{proposition}$[\alpha]\vee[\alpha]=\frac{1}{p}\gamma([NS])\alpha([Sx_{q-1}x_q])=\frac{q}{p}.$\end{proposition}
...
\noindent Thus a necessary condition for two lens spaces $L(p;q_1)$ and $L(p;q_2)$ to be homeomorphic is that $q_1\equiv \pm q_2^{\pm 1}\text{mod }p$.\\
