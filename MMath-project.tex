...
\begin{definition}
Let $p$ and $q\in\mathbb{N}$ be coprime. Then the \textbf{Lens space} $L(p;q)$ is the quotient of $S^3$ by $\mathbb{Z}_p$ with the action generated by $(v,w)\mapsto(e^{2\pi i/p}v,e^{2\pi i/p}w)$
\end{definition}
...
Now let $(a,b)\in S^3$. If $S=(B_{\text{sin}(\frac{\pi}{p})}(a)\cap S^1,B_{\text{sin}(\frac{\pi q}{p})}(b)\cap S^1)$ then $gS\cap S=\emptyset$ for all non-trivial $g\in \mathbb{Z}_p$ so the action is a covering space action giving $\pi_1(L(p;q))\cong \mathbb{Z}_p$. $H_1(L(p;q))\cong \mathbb{Z}_p$ by the Hurewicz theorem.
\end{proof}
