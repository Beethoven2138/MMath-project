...
\noindent Consider a filled-in ball with points N and S corresponding to the north and south poles and points $x_1,...,x_p$ being evenly-spaced points along the equator. Then draw a line from N to S, lines connecting $x_i$ to $x_{i+1}$, lines connecting $x_i$ to N and lines connecting $x_i$ to S. We then identify the faces on the surface by twisting the top hemisphere by $\frac{2\pi iq}{p}$ radians and projecting downwards. This leaves us, after identification, with:\\
...
$\partial[Nx_iS]=[x_iS]-[NS]+[Nx_i]=-[Nx_{i-q}]-[NS]+[Nx_i]=0$\\
$\partial[Nx_iSx_{i+1}]=[x_iSx_{i+1}]-[NSx_{i+1}]+[Nx_ix_{i+1}]-[Nx_iS]$\\
$=-[Nx_{i-q}x_{i+1-q}]+[Nx_{i+1}S]+[Nx_ix_{i+1}]-[Nx_iS]$

\noindent $\partial\sum_i[Nx_0Sx_1]=\sum_i([Nx_ix_{i+1}]-[Nx_{i-q}x_{i+1-q}])+\sum_i([Nx_{i+1}S]-[Nx_iS])=0$ so $H_3(L(p;q);\mathbb{Z})$ is generated by $\sum_i[Nx_0Sx_1]$.\\
$[NS]=[Nx_q]+[x_qS]=[Nx_q]-[Nx_0]$\\
$\partial\sum_{i=0}^{q-1}[Nx_ix_{i+1}]=(q-1)[x_0x_1]+[Nx_0]-[Nx_q]=(q-1)[x_0x_1]-[NS]$\\
\noindent $\partial p[Nx_ix_{i+1}]=p[x_0x_1]-\sum_i(-[Nx_{i+1}]+[Nx_i])=p[x_0x_1]$ so $[x_0x_1]$ is a generator of $H_1(L(p;q);\mathbb{Z})$.\\
\end{document}
