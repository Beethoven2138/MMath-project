...
\begin{theorem}
(Poincare Duality). If $M$ is a closed $R$-orientable $n$-manifold with fundamental class $[M]\in H_n(M;R)$, then the map $D\colon H^k(M;R)\to H_{n-k}(M;R)$ defined by $D([\alpha])=[\alpha]\cap[M]$ is an isomorphism for all $k$.
\end{theorem}
...
$\partial[Nx_0x_{p-1}]=[x_0x_{p-1}]-[Nx_{p-1}]+[Nx_0]=[Nx_0]-[Nx_{p-1}]-[x_0x_1]$.\\
Otherwise, $\partial[Nx_ix_{i+1}]=[x_0x_1]-[Nx_{i+1}]+[Nx_i]$.\\
$\partial[NSx_i]=[Sx_i]-[Nx_i]+[NS]=[NS]-[Nx_i]+[Nx_{i-q}].$\\
$\partial[NSx_0x_{p-1}]=[Sx_0x_{p-1}]-[Nx_0x_{p-1}]+[NSx_{p-1}]-[NSx_0]=-[Nx_{p-1-q}x_{-q}]-[Nx_0x_{p-1}]+[NSx_{p-1}]-[NSx_0]$.\\
Otherwise, $\partial[NSx_ix_{i+1}]=[Sx_ix_{i+1}]-[Nx_ix_{i+1}]+[NSx_{i+1}]-[NSx_i]=[Nx_{i-q}x_{i+1-q}]-[Nx_ix_{i+1}]+[NSx_{i+1}]-[NSx_i]$.\\
...
We also have $\partial b_i=d-f_{i+1}+f_i$ and $\partial c_i=e-f_i+f_{i-q}$, indexed modulo $p$.\\
$d\delta_d=\sum_{i=0}^{p-1}b_i$,\\
$d\delta_e=\sum_{i=0}^{p-1}c_i$,\\
$d\delta_{f_i}=\delta_{b_i}-\delta_{b_{i-1}}-\delta_{c_i}+\delta_{c_{i+q}}$.\\
Then define $\gamma=\delta_d + (\sum_{i=0}^{p-1}i\delta_{f_i})+q\delta_e\in C^1(L(p;q);\mathbb{Z})$.\\
$d\gamma=\sum_{i=0}^{p-1}\delta_{b_i}+(\sum_{i=0}^{p-1}i(\delta_{b_i}-\delta_{b_{i-1}}-\delta_{c_i}+\delta_{c_{i+q}}))+\sum_{i=0}^{p-1}q\delta_{c_i}$\\
$=\sum_{i=0}^{p-1}((i+1)\delta_{b_i}-i\delta_{b_{i-1}}-(i-q)\delta_{c_i}+i\delta_{c_{i+q}})\equiv 0\text{ mod }p$.\\
...
$\gamma(e)=q$.\\
$p\alpha([Sx_{q-1}x_q])=p\alpha([Nx_{p-1}x_0])=p\alpha(b_{p-1})=(p-1+1)\delta_{b_{p-1}}(b_{p-1})-0\times\delta_{b_{-1}}(b_{-1})=p\implies \alpha([Sx_{q-1}x_q])=1$.\\
Otherwise, $p\alpha([Sx_ix_{i+1}])=p\alpha([Nx_{i-q}x_{i+1-q}])=p\alpha(b_{i-q})=(kp+i-q+1)\delta_{b_{{i-q}}}(b_{{i-q}})-(kp+i-q+1)\delta_{b_{{i-q}}}(b_{{i-q}})=0$, where $k\in\mathbb{Z}$ puts $kp+i-q+1$ in the compatible range for the sum of $[1,p-1]$ (The fact that it can't wrap around to $0$ is why cancellation occurs).\\
