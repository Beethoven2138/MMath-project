...
\noindent This makes $f^*$ a ring homomorphism and also shows the functoriality of the cup product needed for the torsion linking form to be a homotopy-invariant.
...
(Here we use $[X]$ to refer to the composition of the quotient map with a singular simplex in $S^3$ and the reason for using $[NSx_0x_{p-1}]$ as opposed to $[NSx_{p-1}x_0]$, for example, is to comply with condition $2$ of the definition of a delta complex)\\
...
\begin{theorem}
Let $X$ and $Y$ be topological spaces, let $f:X\to Y$ be a homeomorphism and let $(A,\beta)$ be a representation of $\pi_1(Y)$. Then $\Delta(X;\beta\circ f_*)=\Delta(Y;\beta)$
\end{theorem}
...
It can also be shown that \[\Delta(L(p;q);\beta_\omega)=\Delta(L(p;q^{-1});\beta_\omega).\]
