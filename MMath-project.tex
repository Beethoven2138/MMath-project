...
Let $\alpha=\frac{1}{p}d\gamma\in C^2(L(p;q);\mathbb{Z})$. Then $p\alpha$ is a representative of an element in $H^2(L(p;q);\mathbb{Z})$.\\
$\gamma(e)=q$.\\
$p\alpha([Sx_{q-1}x_q])=p\alpha([Nx_{p-1}x_0])=p\alpha(b_{p-1})=(p-1+1)\delta_{b_{p-1}}(b_{p-1})-0\times\delta_{b_{p-1}}(b_{p-1})=p\implies \alpha([Sx_{q-1}x_q])=1$.\\
Otherwise, $p\alpha([Sx_ix_{i+1}])=p\alpha([Nx_{i-q}x_{i+1-q}])=p\alpha(b_{i-q})=(kp+i-q+1)\delta_{b_{{i-q}}}-(kp+i-q+1)\delta_{b_{{i-q}}}=0$, where $k\in\mathbb{Z}$ puts $kp+i-q+1$ in the compatible range for the sum of $[1,p-1]$.\\
